...
const datum = dayjs(film.premiera);

premieraElement.innerHTML = `Premiéra <strong>${datum.format(
  'D. M. YYYY',
)}</strong>`;

const dnes = dayjs();
const rozdilDnu = dayjs(datum).diff(dayjs(), 'days');
...
if (rozdilDnu === 0) {
  ...
  premieraElement.innerHTML = `Premiéra <strong>${datum.format(
    'D. M. YYYY',
  )}</strong>, což bylo před ${-datum.diff(dnes, 'days')} ${dnyRetezec}.`;
} else {
  if (rozdilDnu === 1) {
    dnyRetezec = 'dnem';
  ...
  premieraElement.innerHTML = `Premiéra <strong>${datum.format(
    'D. M. YYYY',
  )}</strong>, což bude za ${datum.diff(dnes, 'days')} ${dnyRetezec}.`;
}
